...
void MainWindow::on_connectButton_clicked()
{
    ...
    ui->outputText->appendPlainText(QString("Subscribing to topic "+ newTopic + " for client " CLIENTID));
    int x = MQTTClient_subscribe(client, newTopic.toStdString().c_str(), QOS);
...
void MainWindow::on_MQTTmessage(QString payload){
    ...
    QJsonDocument txt = QJsonDocument::fromJson(json.toUtf8());
    QJsonObject obj = txt.object();
    QJsonArray arr = obj["Pitch"].toArray();
    qDebug()<<"Pitch:"<<txt["Pitch"].toDouble();
    this->count=txt["Pitch"].toDouble();
    this->update();
    //ADD YOUR CODE HERE
}
...
void MainWindow::on_changeTopic_clicked()
{
    QString topicText = ui->inputTopic->toPlainText();
        qDebug()<<"Topic Text"<< topicText;
        newTopic = topicText;
}
